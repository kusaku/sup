...
class FSDocumentAct extends FSPDFDocument implements IDocumentHtmlResult {
	const VERSION='3';
	protected $sType='act';
	protected $obPackage;
	...
	protected function getController() {
		$arResult=Yii::app()->createController('documents/document/act');
	...
	public function getTitle() {
		return 'Акт о выполненных работах №'.$this->obPackage->getNumber();
	}
	...
	public function getHash() {
		if($this->sHash=='') {
			$arData=array();
			$arData['package']=$this->obPackage->getNumber();
	...
	public function getAsHtml() {
		if($obOldDocument=$this->obStorage->getModel()->findByAttributes(array('md5summ'=>$this->getHash(),'type'=>$this->sType,'storage_format'=>'html'))) {
			...
			$obController->createAction('act')->runWithParams(array('package'=>$this->obPackage,'jur_person'=>$this->obJurPerson,'client_jur_person'=>$this->obClientJurPerson,'services'=>$this->arServices));
