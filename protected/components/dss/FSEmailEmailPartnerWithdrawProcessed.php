...
class FSEmailEmailPartnerWithdrawProcessed extends FSEmail {
	protected $sType='emailPartnerWithdrawProcessed';
	protected $obUser;
	...
	function __construct($obStorage,$obWithdraw) {
		...
		$this->obUser=$this->obWithdraw->partner_data->user_data;
	}
	...
	protected function getController() {
		$arResult=Yii::app()->createController('documents/email/partnerwithdrawprocessed');
	...
	public function _getAsHtml() {
		if($obController=$this->getController()) {
			$obController->createAction('partnerwithdrawprocessed')->runWithParams(array('withdraw'=>$this->obWithdraw,'user'=>$this->obUser));
