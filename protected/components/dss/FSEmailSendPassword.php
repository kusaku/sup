 ...
 */
class FSEmailSendPassword extends FSEmail {
	protected $sType='emailSendPassword';
	protected $obUser;
	...
	function getTitle() {
		return 'Новый логин и пароль для доступа в Личный кабинет клиента';
	}
	...
	protected function getController() {
		$arResult=Yii::app()->createController('documents/email/sendpassword');
	...
	public function _getAsHtml() {
		if($obController=$this->getController()) {
			$obController->createAction('sendpassword')->runWithParams(array('user'=>$this->obUser,'pwd'=>$this->sOpenPassword));
