...
class FSEmailRegister extends FSEmail {
	protected $sType='emailRegister';
	protected $obUser;
	...
	function getTitle() {
		return 'Логин и пароль для доступа в Личный кабинет клиента';
	}
	...
	protected function getController() {
		$arResult=Yii::app()->createController('documents/email/register');
	...
	public function _getAsHtml() {
		if($obController=$this->getController()) {
			$obController->createAction('register')->runWithParams(array('user'=>$this->obUser,'pwd'=>$this->sOpenPassword));
