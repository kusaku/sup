	...
	<div class="wrapper">
		<div class="editClientWindow" id="sm_content">
			...
			<div class="clientHead">
				Просмотр заказа в работе #<?= $pack->id?>
			</div>
			...
			<div class="domainInfo">
				...
				<div id="site_selector">
					<strong>Сайт:</strong>
					<?= $pack->site_id ? $pack->site->url : sites($client->id, $pack->site_id).'<a href="javascript:loadNewSite();" class="plus">+</a>'; ?>
					<br></div>
					...
					foreach ($tabs as $tab) {
						...
						if ($tab['to_redmine']) {
							$this->renderPartial('issue', array(
								'issue_id'=>$tab['to_redmine'],'pack_id'=>$pack->id,'serv_id'=>$tab['serv_id']
							));
						} else {
