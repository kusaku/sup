...
<div class="wrapper">
	...
	<div class="report">
		...
		<h1 class="expandnext" style="float:left;clear:both;cursor:pointer;border-bottom:1px dashed #FF7B0B;"><?= $managerItem['name']?></h1>
		<table class="reportItem">
			<tr style="display:none;" class="collapsible">
				...
				<th style="width: 10%;text-align:right;">За&nbsp;исключением</th>
				<th style="width: 10%;text-align:right;">Итого</th>
			</tr>
			<?php foreach ($managerItem['pays'] as $payItem): ?>
			<tr style="display:none;" class="collapsible">
				<td><?= $payItem['dt']?></td>
				<td><?= $payItem['client']?><?= empty($payItem['mail']) ? '' : " ({$payItem['mail']})"?></td>
				<td><?= $payItem['description']?></td>
				<td><?= $payItem['name']?><?= empty($payItem['site']) ? '' : " ({$payItem['site']})"?> (<?php echo $payItem['id']?>)</td>
				<td style="text-align:right;"><?= number_format($payItem['amount'], 0, ',', ' ')?>   руб.</td>
				<td style="text-align:right;"><?= number_format($payItem['exclude'], 0, ',', ' ')?>   руб.</td>
				<td style="text-align:right;"><?= number_format($payItem['summ'], 0, ',', ' ')?>   руб.</td>
			</tr>
			<?php endforeach; ?>
			<tr>
				<th>Количество:</th>
				...
				<th style="text-align:right;">
					<?= number_format($managerItem['excluded'], 0, ',', ' ')?>  руб.
				</th>
				<th style="text-align:right;">
					<?= number_format($managerItem['total'], 0, ',', ' ')?>  руб.
				</th>
			</tr>
		</table>
		<?php endforeach; ?>
		...
		<table class="reportItem">
			...
			<tr>
				...
				<th style="width: 10%;text-align:right;">
					<?= number_format($total['excluded'], 0, ',', ' ')?>  руб.
				</th>
				<th style="width: 10%;text-align:right;">
					<?= number_format($total['total'], 0, ',', ' ')?>  руб.
				</th>
			</tr>
		</table>
	</div>
