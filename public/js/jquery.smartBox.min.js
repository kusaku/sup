	...
	$.extend($.fn, {
		...
		selectBox: function(method, data) {
			...
			var showMenu = function(select) {
                ...
                if ($(window).scrollTop() + $(window).height() > control.offset().top + control.outerHeight() + options.outerHeight()) {
                    var top = control.offset().top + control.outerHeight();
                }
                else {
                    var top = control.offset().top - options.outerHeight();
                }
				
				// Show menu
				var offscreen = control.offset().top + options.outerHeight()
				options.css({
...
if (jQuery) 
    (function($){
        $.extend($.fn, {
            tipBox: function(method, data){
                var init = function(element, data){
                    element = $(element);
                    
                    var control;
                    
                    if (control = element.data('tipBox-control')) {
                        control.find('span').text(data);
                        return false;
                    }
                    
                    control = $('<div class="tipBox"><div style="position:relative"/></div>').append($('<span/>').text(data || element.attr('title') || '!')).append($('<div style="position:absolute" class="tip"/>'));
                    control.css('position', 'absolute').hide().appendTo('body');
                    element.data('tipBox-control', control);
                }
                
                var rebase = function(element){
                    element = $(element);
                    var control = element.data('tipBox-control');
                    if (!control) 
                        return;
                    
                    control.removeClass('contra bottom top right left')
                    
                    var top, left, className;
                    
                    if ($(window).scrollTop() + $(window).height() < element.offset().top + element.outerHeight() + control.outerHeight()) {
                        top = element.offset().top - control.outerHeight();
                        className = 'top';
                    }
                    else 
                        if ($(window).scrollTop() > element.offset().top) {
                            ...
                            top = element.offset().top + parseInt((element.outerHeight() - control.outerHeight()) / 2);
                            className = 'contra';
                        }
                    
                    control.addClass(className);
                    
                    if ($(window).scrollLeft() + $(window).width() < element.offset().left + element.outerWidth() + control.outerWidth()) {
                        left = element.offset().left - control.outerWidth();
                        className = 'left'
                    }
                    else 
                        if ($(window).scrollLeft() > element.offset().left || className == 'contra') {
                            left = element.offset().left + element.outerWidth();
                            className = 'right'
                        }
                        else {
                            left = element.offset().left + parseInt((element.outerWidth() - control.outerWidth()) / 2);
                            className = 'contra'
                        }
                ...
                var show = function(element){
                    ...
                    if (!control) 
                        return;
                    $(document).one('mousedown.tipBox', function(){
                        element.tipBox('hide');
                    });
                    rebase(element);
                    control.show();
                ...
                var destroy = function(element){
                    ...
                    if (!control) 
                        return;
                    control.remove();
                    element.removeClass('tipBox').removeData('tipBox-control').removeData('tipBox-radios').show();
                ...
                switch (method) {
                    ...
                    case 'control':
                        return $(this).data('tipBox-control');
                        break;
